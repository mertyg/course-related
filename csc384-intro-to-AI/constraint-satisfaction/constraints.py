...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for posi in qi.curDomain():
            for posj in qj.curDomain():
                if (abs(posi - posj) != abs(i - j)) and (posi!=posj):
                    satisfyingAssignments.append([posi,posj])
    ...
    def check(self):
        ...
        for var in self.scope:
            if var.getValue() in self._required:
                i+=1
        return i<=self._ub and i>=self._lb
    def hasSupport(self, var, val):
        '''check if var=val has an extension to an assignment of the
           other variable in the constraint that satisfies the constraint
        ...
        prospect = 0
        cur = 0
        var_flag = int(val in self._required)
        for var in toassign:
            if not var.isAssigned():
                for dom in var.curDomain():
                    if dom in self._required:
                        prospect+=1
                        break
            else:
                if var.getValue() in self._required:
                    cur+=1
        if cur+var_flag>self._ub:
            return False
        elif cur+prospect+var_flag<self._lb:
            return False
        return True
