...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint(str(i),row))
        i+=1
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint("{}{}{}".format(i,j,k),scope))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    #field: planes, flights,_can_fly, _flights_at_start, can_follow, maintenance_flights
    #min_maintenance_frequency
    constraint_list = []
    variables = [[] for i in range(len(planes_problem.planes))]
    all_variables = []
    compatible = planes_problem.can_follow

    for i,plane in enumerate(planes_problem.planes):
        variables[i] = []
        dom = planes_problem._can_fly[plane]
        for j in range(len(dom)):
            if j==0:
                do = list(set(planes_problem._can_fly[plane]).intersection(planes_problem._flights_at_start[plane]))
                var = Variable("{}{}".format(plane,str(i)),do+["None"])
            else:    
                var = Variable("{}{}".format(plane,str(i)),dom+["None"])
            variables[i].append(var)
            all_variables.append(var)


    ##each flight mustbe scheduled and exactly once
    for f in planes_problem.flights:
        c = NValuesConstraint("Flight {}".format(f), all_variables, [f], 1, 1)
        constraint_list.append(c)

    ##maintenance 
    for i in range(len(variables)):
        windows = len(variables[i])-planes_problem.min_maintenance_frequency+1
        if windows>0:
            for j in range(windows):
                scope = []
                for k in range(j,j+planes_problem.min_maintenance_frequency):
                    scope.append(variables[i][k])
                c = NValuesConstraint("Maint_Check {}{}".format(i,k), scope,planes_problem.maintenance_flights +["None"] , 1,planes_problem.min_maintenance_frequency)
                constraint_list.append(c)

    #sequence
    for f in planes_problem.flights:
        compatible.append((f,"None"))
    compatible.append(("None","None"))

    for i in range(len(variables)):
        windows = len(variables[i])-2+1
        for j in range(windows):
            scope = [variables[i][j],variables[i][j+1]]
            c = TableConstraint("Seq_Check {}{}".format(i,j), scope,compatible)
            constraint_list.append(c)


    csp = CSP("PlaneProb",all_variables,constraint_list)
    ...
    for solution in solutions:
        ...
        for i in range(len(variables)):
            var = []
            var.append(planes_problem.planes[i])
            for j in range(len(variables[i])):
                if solution[cur][1]!="None":
                    var.append(solution[cur][1])
                cur+=1
            proper.append(var)
        to_return.append(proper)
    return to_return
