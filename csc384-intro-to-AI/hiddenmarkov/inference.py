...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i%total])
        "*** END YOUR CODE HERE ***"
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                allPossible[p]+=emissionModel[dist]*old[p]
            if not any(allPossible.values()):
                self.initializeUniformly(gameState)
            else:
                observed = []
                for i in range(self.numParticles):
                    observed.append(util.sample(allPossible))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        samplefrom = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(samplefrom)
        for i in range(self.numParticles):
            self.particles.append(samplefrom[i%len(samplefrom)])
        "*** END YOUR CODE HERE ***"
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    ...
                    current *= emissionModels[i][trueDist]
            allPossible[particle]+=current
